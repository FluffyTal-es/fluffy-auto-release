...
const createReleaseTag = async (client: github.GitHub, refInfo: Octokit.GitCreateRefParams) => {
  ...
  try {
    ...
    core.info(
      `Could not create new tag "${refInfo.ref}" (${err.message as string}) therefore updating existing tag "${existingTag}"`,
    );
...
const deletePreviousGitHubRelease = async (client: github.GitHub, releaseInfo: Octokit.ReposGetReleaseByTagParams) => {
  ...
  try {
    ...
    core.info(`Could not find release associated with tag "${releaseInfo.tag}" (${err.message as string})`);
  }
  core.endGroup();
};
...
const getCommitsSinceRelease = async (
  ...
  try {
    ...
    core.info(
      `Could not find SHA corresponding to tag "${tagInfo.ref}" (${err.message as string}). Assuming this is the first release.`,
    );
